...
* Сделать через pydantic settings 
```
...
- [x] Подними asyncio TCP‑сервер (asyncio.start_server) и выведи в лог входящие соединения.
    
- [x] Реализуй минимальный парсер HTTP‑запроса: стартовая строка (метод, путь, версия) + заголовки (без сложных кейсов, достаточно CRLF). Тело — передавать как raw‑стрим.
    
- [x] Сделай подключение к одному апстриму (asyncio.open_connection), проксируй запрос и ответ (двунаправленно), соблюдая backpressure через drain ().
    
- [x] Добавь таймауты (asyncio.wait_for) на этапы connect/read/write, и общий total.
    
- [x] Подключи балансировку round‑robin по нескольким апстримам.
    
- [x] Введи лимиты на количество одновременных соединений/запросов и соединений к апстриму (например, через asyncio. Semaphore).
    
- [x] Добавь логирование и минимальные метрики (счётчики/таймеры). Опционально — отдельная /metrics ручка на другом порту. Дополнительно создать некий tarce_id для каждого запроса, чтобы в логах и метриках можно было проследить весь его путь и этапы прохождения от клиента до апстрима и обратно.
    

Доп:
- [x] Добавить какой-то trace_id для прослеживаемости одного запроса через все этапы проксирования в логах.
- [x] Метрики (кол-во запросов, ответов, ошибок) (отдельный сервис, который это все аккумулировать, и по ручке отдавать отчет)
- [x] Конфиг из файла. Горячая перезагрузка конфигурации (SIGHUP) без остановки сервера.
...
> Задача сделать больше 10к RPS
> Егор Рогов Ютуб про Postgres PRO
> 
