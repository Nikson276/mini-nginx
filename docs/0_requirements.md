...
- [ ] Добавь логирование и минимальные метрики (счётчики/таймеры). Опционально — отдельная /metrics ручка на другом порту. Дополнительно создать некий tarce_id для каждого запроса, чтобы в логах и метриках можно было проследить весь его путь и этапы прохождения от клиента до апстрима и обратно.
